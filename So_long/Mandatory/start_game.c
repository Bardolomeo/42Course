...
/*   Updated: 2023/12/03 17:34:25 by gsapio           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	upload_sprites(t_mlx *mlx)
{
	...
	mlx->imgs[6].img = mlx_xpm_file_to_image(mlx->mlx_ptr, IDLE_2,
			&mlx->imgs[6].wid, &mlx->imgs[6].hei);
	mlx->imgs[7].img = mlx_xpm_file_to_image(mlx->mlx_ptr, ENEMY_1,
			&mlx->imgs[7].wid, &mlx->imgs[7].hei);
	mlx->imgs[8].img = mlx_xpm_file_to_image(mlx->mlx_ptr, ENEMY_2,
			&mlx->imgs[8].wid, &mlx->imgs[8].hei);
	mlx->imgs[9].img = NULL;
}
